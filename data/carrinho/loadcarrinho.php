...
if ( $_POST[ 'carregarcarrinho' ] = 'sim' ) {
  ...
  if ( mysqli_num_rows( $carrinho ) > 0 ) {
	...
	if($tipoproduto == '9'){$tipoproduto= "Alaminuta Simples";}	
      ...
      $resposta .= "<tr align='center'><td align='center'>$tipoproduto</td><td align='center'>$nomeproduto</td><td align='center'><input class='form-control text-center' id='valorupg$idcarrinho' value='$valor' onkeyup='substituiPonto(this)' onChange='editproduto($idcarrinho)'/><td><a href='#' onClick='delproduto($idcarrinho);'><i class='fa fa-trash fa-fw'></i></a></td></tr>";  
	}

    $resposta .= "
  </tbody></table></div>&nbsp;<input class='form-control text-center' id='obs' placeholder='Observações'/>&nbsp;";
    ...
    $resposta.= "	  	<select class='form-control' aria-label='Default select example' id='plataforma' onChange='buttonvalid()'>
						<option value='Não Selecionado' selected>Selecione a Plataforma</option>";
    ...
    while ( $lmotoboy = mysqli_fetch_array( $consultamotoboy ) ) {
      ...
      $resposta .= "	
                		<option value='$identregador'>$nomeentregador</option>
						";
    }
    $resposta .= "	
						</select><br>
